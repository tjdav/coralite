 ...
 * @import {CoralitePlugin} from './plugin.js'
 */
 ...
 * @param {CoralitePlugin[]} [options.plugins=[]]
...
export function Coralite ({
  ...
  this.pages = getHtmlFiles({
    ...
    onFileUpdate (newValue, oldValue) {
      ...
      if (!newValue.result) {
        const result = parseHTML(newValue.content, ignoreByAttribute)

        newCustomElements = result.customElements
      } else {
 ...
 * @return {Promise<Array<CoraliteResult>>} - An array of objects containing:
    ...
    for (let i = 0; i < document.customElements.length; i++) {
      ...
      if (!values) {
        if (typeof customElement.attribs === 'object') {
          ...
          values = Object.assign({}, document.values)
        }
...
Coralite.prototype.createComponent = async function ({
  ...
  for (let i = 0; i < customElements.length; i++) {
    ...
    if (typeof customElement.attribs === 'object') {
      ...
      this.values[childContextId] = Object.assign({}, values)
    }
        ...
        for (let i = startIndex; i > -1; i--) {
          ...
          if (component) {
            ...
            if (typeof node.attribs === 'object') {
              ...
              this.values[slotContextId] = Object.assign({}, values)
            }
...
Coralite.prototype._evaluate = async function ({
  ...
  const parentPath = resolve(join(this.path.templates, template.path.pathname))

  if (!isAbsolute(parentPath)) {
    throw new Error(`Resolved parent path must be absolute but received: ${parentPath}. Check template pathname or base directory configuration`)
  }

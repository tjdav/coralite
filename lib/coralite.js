...
Coralite.prototype._evaluate = async function ({
  ...
  let parentPath = template.path.pathname

  // Resolve the parent path of the template

  if (!existsSync(parentPath)) {
    parentPath = resolve(join(this.path.templates, template.path.pathname))

    if (!existsSync(parentPath)) {
      throw new Error('Unexpected template pathname: ' + parentPath)
    }
  }

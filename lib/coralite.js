 ...
 *  CoraliteModule,
 *  CoraliteResult,
 *  CoraliteDocumentCollectionItem,
 ...
 * @param {string|CoraliteDocumentCollectionItem} value - The path to a page or a CoraliteDocumentCollectionItem to add to the render queue.
 */
      ...
      if (!slotNodes.length) {
        ...
        for (let i = startIndex; i > -1; i--) {
          ...
          if (component) {
            const component = await this.createComponent({
              id: node.name,
              values: Object.assign(values, node.attribs),
              element: node,
              document
            }, false)
...
Coralite.prototype._evaluate = async function ({
  ...
  const parentPath = resolve(template.path.page)
  const __dirname = parentPath.substring(0, parentPath.length - template.path.page.length - 1)

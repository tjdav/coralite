...
export function Coralite ({
  ...
  // place core defineComponent plugin first
  ...
  for (let i = 0; i < plugins.length; i++) {
    ...
    // Bind the plugin method to the current instance.
    const callback = plugin.method.bind(this)
    const name = plugin.name
    const _self = this._options

    // add an export for each plugin in the generated modules.
    _self.source.modules.plugins.export += `export const ${name} = coralite.plugins.${name};\n`
    _self.source.modules.plugins.default += name + ', '

    // extend the source context with a reference to the plugin method.
    _self.source.context.plugins[name] = function (options) {
      return callback(options, _self.source.contextInstances[_self.source.currentSourceContextId])
    }

    // queue any templates provided by the plugin to be registered.
    for (let i = 0; i < plugin.templates.length; i++) {
      _self.plugins.templates.push(plugin.templates[i])
    }

    // add the plugin's hooks to the appropriate Coralite hook lists.
    if (plugin.onPageCreate) {
      this._addPluginHook('onPageCreate', plugin.onPageCreate)
    }
    ...
    if (plugin.onTemplateCreate) {
      this._addPluginHook('onTemplateCreate', plugin.onTemplateCreate)
    }
  ...
  this._options.source.modules.plugins.default = this._options.source.modules.plugins.default.substring(0, this._options.source.modules.plugins.default.length - 2) + ' }'
  this.values = {}
...
Coralite.prototype.initialise = async function () {
  this.templates = await getHtmlFiles({
    path: this._options.templates,
    recursive: true,
    type: 'template',
    onFileSet: async (value) => {
      const template = parseModule(value.content, {
        ignoreByAttribute: this._options.ignoreByAttribute
      })

      await this._triggerPluginHook('onTemplateCreate', template)
  ...
  for (let i = 0; i < this._options.plugins.templates.length; i++) {
    await this.templates.setItem(this._options.plugins.templates[i])
  }
  ...
  const onFileSet = async (data) => {
    const elements = parseHTML(data.content, this._options.ignoreByAttribute)
    ...
    await this._triggerPluginHook('onPageCreate', {
      elements,
      values,
      data
    })
  ...
  this.pages = await getHtmlFiles({
    ...
    onFileUpdate: async (newValue, oldValue) => {
      ...
      this._triggerPluginHook('onPageUpdate', {
...
Coralite.prototype.compile = async function (path) {
  ...
  if (Array.isArray(path)) {
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      if (pathname.startsWith(this._options.path.pages)) {
        pathname = pathname.substring(this._options.path.pages.length - 1)
      }
    ...
    if (path.startsWith(this._options.path.pages)) {
      path = path.substring(this._options.pages.length - 1)
    }
...
Coralite.prototype.getPagePathsUsingCustomElement = function (path) {
  ...
  if (path.startsWith(this._options.path.templates)) {
    path = path.substring(this._options.path.templates.length + 1)
  }
...
Coralite.prototype.createComponent = async function ({
  ...
  if (!templateItem) {
    return console.warn('Could not find component "' + id + '" used in document "' + join(this._options.path.pages, document.path.pathname) + '"')
  }
...
Coralite.prototype._evaluate = async function ({
  ...
  this._options.source.currentSourceContextId = contextId

  const context = {
    ...this._options.source.context,
    document,
    values,
    element,
    module,
  ...
  this._options.source.contextInstances[contextId] = context
  ...
  if (!existsSync(parentPath)) {
    parentPath = resolve(join(this._options.path.templates, template.path.dirname))
...
Coralite.prototype._moduleLinker = function (path) {
  ...
  return async (specifier, referencingModule, extra) => {
    if (specifier === 'coralite') {
      return new SourceTextModule(self._options.source.modules.coralite.export + self._options.source.modules.coralite.default, {
        context: referencingModule.context
      })
    } else if (specifier == 'coralite/plugins') {
      return new SourceTextModule(self._options.source.modules.plugins.export + self._options.source.modules.plugins.default, {
        context: referencingModule.context
      })
 ...
 * @param {'onPageCreate'|'onPageUpdate'|'onPageDelete'|'onTemplateCreate'|'onTemplateUpdate'|'onTemplateDelete'} name - The name of the hook to trigger.
 ...
 * @param {'onPageCreate'|'onPageUpdate'|'onPageDelete'|'onTemplateCreate'|'onTemplateUpdate'|'onTemplateDelete'} name - The name of the hook to register the callback with.
...
Coralite.prototype._addPluginHook = function (name, callback) {
  ...
  if (this._options.plugins.hooks[name]) {
    this._options.plugins.hooks[name].push(pluginCallback)
  }
}

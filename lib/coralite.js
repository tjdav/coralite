...
export function Coralite ({
  ...
  for (let i = 0; i < plugins.length; i++) {
    ...
    for (let i = 0; i < plugin.templates.length; i++) {
      options.plugins.templates.push(plugin.templates[i])
    }
...
Coralite.prototype.initialise = async function () {
  ...
  for (let i = 0; i < this.options.plugins.templates.length; i++) {
    await this.templates.setItem(this.options.plugins.templates[i])
  }
...
Coralite.prototype._triggerPluginHook = async function (name, data) {
  ...
  const pluginHooks = this.options.plugins.hooks[name]
...
Coralite.prototype._addPluginHook = function (name, callback) {
  ...
  if (this.options.plugins.hooks[name]) {
    this.options.plugins.hooks[name].push(pluginCallback)
  }
}

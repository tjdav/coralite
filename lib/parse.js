                ...
                if (children.length) {
                  if (children.length === 1 && children[0].type === 'text') {
                    ...
                    for (let i = 0; i < result.customElements.length; i++) {
                      ...
                      replaceCustomElementWithTemplate(customElement, components, document)
                    }
          ...
          try {
            ...
            for (let i = 0; i < results.length; i++) {
              ...
              if (typeof result === 'string') {
                const result = parseHTML(results[i], document.ignoreByAttribute)
                ...
                if (result.root.children.length) {
                  ...
                  tokens[key] = result.root.children
                }
              } else {
                tokens[key] = result
              }
 ...
 * @param {Object.<string, CoraliteModule>} components - A map of component names to their modules.
 * @param {CoraliteDocument} document - The document containing the element.
 */
export async function replaceCustomElementWithTemplate (coraliteElement, components, document) {
  // Create a component instance from the custom element and its attributes
  const component = await createComponent({
    id: coraliteElement.name,
    values: coraliteElement.attribs,

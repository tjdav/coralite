  ...
  const contextifiedObject = vm.createContext({
    ...
    coralite: {
      ...
      async aggregate (options) {
        if (options.template && options.template.item) {
          const component = components[options.template.item]

          if (!component) {
            throw new Error('Aggregate: no component found by the id: ' + options.template.item)
          }

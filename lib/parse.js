  ...
  const parser = new Parser({
    onopentag (originalName, attributes) {
      ...
      if (element.name === 'template') {
        ...
        if (attributes.id.length > 1000) {
          throw new Error("Template ID is too long");
        }
...
export function createElement ({
  ...
  if (!validTags[sanitisedName]) {
    // check if the sanitised name is an invalid custom tag
    ...
    if (sanitisedName.length > 1000) {
      throw new Error("Custom element tag name too long");
    }

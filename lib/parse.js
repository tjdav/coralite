 ...
 * ```
 */
export function parseModule (string, { ignoreByAttribute, html }) {
  // root element reference
  /** @type {CoraliteDocumentRoot} */
  const root = {
    type: 'root',
  ...
  if (html) {
    coraliteModule.filename = html.name
    coraliteModule.parentPath = html.parentPath
  }

  return coraliteModule
}
...
export async function parseScript ({
  ...
  const parentPath = resolve(component.parentPath)
  const __dirname = parentPath.substring(0, parentPath.length - component.parentPath.length - 1)
  const script = new SourceTextModule(component.script, {
    initializeImportMeta (meta) {
      meta.url = __dirname
    },
    identifier: join(parentPath, component.filename),
